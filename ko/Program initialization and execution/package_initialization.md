...
패키지 안에 있는 패키지 레벨 변수들은 *선언된 순서*에 따라 초기화된다. 단, 해당 변수들과 *종속 관계(depend on)* 가 있는 변수들은 먼저 초기화되어야 한다.
...
의존성에 대한 분석은 변수들의 실제값들에 의지하지 않고, 단지 소스내 변수들을 언급하는 어휘적 *레퍼런스*(lexical references)에 의해 전이적으로 (transitively) 분석된다. 예를 들어, 만약에 변수 `x`의 초기화 식이 함수를 언급하고 함수의 몸통이 변수 `y`를 언급한다면, `x`는 `y`에 의존하는 것이다. 구체적으로:
  ...
  * 변수, 함수, 혹은 메서드 `x`가 변수 `y`에 의존적이라 함은 `x`의 초기화 식이나 (함수들이나 메서드들의) 본문이 `y`에 대해 레퍼런스를 가지고 있던지 `y`에 의존적인 함수나 메서드에 레퍼런스를 가지고 있는 경우다.
...
또한, 변수는 패키지 블럭 내 선언된 `init` 이라는 이름의 함수를 사용해 초기화할 수 있다. init 함수는 인수와 결과 매개변수가 없다.
